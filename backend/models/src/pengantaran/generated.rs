...
impl Pengantaran {
    ...
    pub async fn create(db: &mut Connection, item: &CreatePengantaran) -> QueryResult<Self> {
        ...
        insert_into(pengantaran).values(item).get_result::<Self>(db).await
    }
    ...
    pub async fn paginate(db: &mut Connection, page: i64, page_size: i64) -> QueryResult<PaginationResult<Self>> {
        ...
        let items = pengantaran.limit(page_size).offset(page * page_size).load::<Self>(db).await?;
    ...
    pub async fn update(db: &mut Connection, param_id: i32, item: &UpdatePengantaran) -> QueryResult<Self> {
        use crate::schema::pengantaran::dsl::*;

        diesel::update(pengantaran.filter(id.eq(param_id))).set(item).get_result(db).await
    }

    pub async fn delete(db: &mut Connection, param_id: i32) -> QueryResult<usize> {
        use crate::schema::pengantaran::dsl::*;

        diesel::delete(pengantaran.filter(id.eq(param_id))).execute(db).await
    }
