...
impl Penarikan {
    ...
    pub async fn create(
        db: &mut Connection,
        item: &CreatePenarikan,
        param_user_id: i32,
    ) -> QueryResult<Self> {
        use crate::schema::penarikan::dsl::*;

        let res = insert_into(penarikan)
            .values(item)
            .get_result::<Self>(db)
            .await;
    ...
    pub async fn paginate_brief(
        ...
        page_size: i64,
        param_user_id: Option<i32>,
    ) -> QueryResult<PaginationResult<PenarikanBrief>> {
        use crate::schema::company;
        use crate::schema::penarikan::dsl::*;
        use crate::schema::user;
        ...
        Ok(crate::penarikan::PaginationResult {
            items,
            total_items,
            page,
            page_size,
    ...
    pub async fn delete(
        ...
        let res = diesel::delete(penarikan.filter(id.eq(param_id)))
            .execute(db)
            .await;
