...
impl PermohonanStudent {
    pub async fn create(
        db: &mut Connection,
        item: &CreatePermohonanStudent,
        param_user_id: i32,
    ) -> QueryResult<Self> {
        use crate::schema::permohonan_student::dsl::*;

        let res = insert_into(permohonan_student)
            .values(item)
            .get_result::<Self>(db)
            .await;
        ...
        if let Err(e) = Log::create(
            ...
            &CreateLog {
                ...
                table_affected: TableRef::PermohonanStudent,
    ...
    pub async fn delete_by_student_and_letter_id(
        ...
        let previous = permohonan_student
        ...
        let res = diesel::delete(permohonan_student.filter(student_id.eq(param_student_id)))
            .filter(permohonan_id.eq(param_letter_id))
            .execute(db)
            .await;
