...
impl Company {
    ...
    pub async fn create(db: &mut Connection, item: &CreateCompany) -> QueryResult<Self> {
        ...
        insert_into(company).values(item).get_result::<Self>(db).await
    }
    ...
    pub async fn paginate(db: &mut Connection, page: i64, page_size: i64) -> QueryResult<PaginationResult<Self>> {
        ...
        let items = company.limit(page_size).offset(page * page_size).load::<Self>(db).await?;
    ...
    pub async fn update(db: &mut Connection, param_id: i32, item: &UpdateCompany) -> QueryResult<Self> {
        use crate::schema::company::dsl::*;

        diesel::update(company.filter(id.eq(param_id))).set(item).get_result(db).await
    }

    pub async fn delete(db: &mut Connection, param_id: i32) -> QueryResult<usize> {
        use crate::schema::company::dsl::*;

        diesel::delete(company.filter(id.eq(param_id))).execute(db).await
    }
